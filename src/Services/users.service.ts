...
export class UsersService {
  ...
  getUserInfo(userId: number): Observable<IUser_Info> {
    ...
    return this.http
      .get(this.server + this.route_userInfo + userId, { headers: reqHeader })
      .pipe(
        map(response => {
  ...
  deleteUserAccount() {
    ...
    return this.http.delete(`${this.server}${this.route_deleteUserAcc}`, {
      headers: reqHeader,
    });
  }
  ...
  updateUserImg(imageFile: File | undefined) {
    ...
    return this.http
      .post(this.server + this.route_userImg, formData, { headers: reqHeader })
      .pipe(
        map(resp => {
          return resp;
        })
