...
export class SerachBarService {
  ...
  initSubFoodditNames() {
    this.subfoodditService
      .getSubfooddits()
      .subscribe((subFooddits: Subfooddit[]) => {
  ...
  searchSubFooddits(searchTerm: string) {
    ...
    const newSearch: string[] = this.allSubFooddits.filter(sf => {
  ...
  addToRecentSearches(searchTerm: string) {
    this.recentSearcher.set([
      searchTerm,
      ...this.recentSearcher().filter(
        s => s?.toLocaleLowerCase() !== searchTerm?.toLocaleLowerCase()
      ),
    ]);
  }
